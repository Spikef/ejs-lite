 ...
 * @returns {*}
 */
exports.render = function(template, options, data) {
...
function handleCache(options) {
    ...
    if ( options.cache ) {
        ...
        if ( !fn ) {
            template = readTemplate(filename, options.encoding);
            TPL = new Template(template, options);
            fn = TPL.compile();
...
function readTemplate(filename, encoding) {
    if ( !fs.existsSync(filename) ) {
        ...
        return fs.readFileSync(filename, encoding || _DEFAULT_ENCODING);
    }
}
 ...
 */
function repeatList(obj, print, close) {
    var arr = [],
        name = obj.name,
        tag = obj.tag,
        end = obj.end;
 ...
 * @returns {string}: 循环体方法字符串
 */
function repeatJSON(obj, print, close) {
    var arr = [],
        name = obj.name,
        tag = obj.tag,
        end = obj.end;
                        ...
                        case Template.modes.CIRCLES:
                            ...
                            if ( args = line.match(/^\s*(?!<)(\S+)\s*([\s\S]*)/) ) {
                                ...
                                if ( args[1].indexOf(ds.loops) === 0 ) {
                                    ...
                                    last.json = true;
                                    this.source += repeatJSON(last, '__append', false) + '\n';
                                } else {
                                    this.source += repeatList(last, '__append', false) + '\n';
                                }
                                ...
                                if ( last.json ) {
                                    this.source += repeatJSON(last, '__append', true) + '\n';
                                } else {
                                    this.source += repeatList(last, '__append', true) + '\n';
                                }
                            }
                            break;
                        // 输出html
